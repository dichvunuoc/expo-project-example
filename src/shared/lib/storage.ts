...
/**
 * Create Zustand storage adapter from MMKV instance
 * This adapter allows Zustand to use MMKV for persistence
 */
export const createZustandStorageAdapter = (
  mmkvStorage: ReturnType<typeof createMMKV>
) => {
  return {
 ...
 * Useful for creating separate storage instances for different features
 */
export const createStorageInstance = (options?: {
  id?: string;
  encryptionKey?: string;
}) => {
  return createMMKV({
    id: options?.id || 'app-storage',
    encryptionKey: options?.encryptionKey || getEncryptionKey(),
  });
};

/**
 * Default Zustand storage adapter using the main storage instance
 * Use this for most Zustand stores
 */
export const zustandStorageAdapter = createZustandStorageAdapter(storage);
...
export class StorageService implements IStorageService {
   ...
   */
  async export(): Promise<Record<string, StorageValue>> {
    try {
      const keys = await this.getAllKeys();
   ...
   */
  async import(data: Record<string, StorageValue>): Promise<void> {
    try {
...
export const AuthStorage = {
  ...
  setUserData: async (userData: StorageValue): Promise<void> => {
    await storageService.set(STORAGE_KEYS.USER_DATA, userData);
  },

  getUserData: async (): Promise<StorageValue | null> => {
...
export const AppStorage = {
  setSetting: async <T extends StorageValue>(
    setting: string,
    value: T
  ): Promise<void> => {
    const currentSettings =
      (await storageService.get<Record<string, StorageValue>>(
  ...
  getSetting: async <T>(setting: string): Promise<T | null> => {
    ...
    return (settings[setting] as T) || null;
  },
